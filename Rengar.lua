...
function Rengar:__init()
  ...
  Rengo.Combo:KeyBinding("empowered_prioritize_key", "Toggle Empowered Priority", string.byte("L"), true, function(priority_change)
...
function Rengar:CastQ(unit, delay, validrange)
	if ValidTarget(unit, validrange) then
	    if CanUseSpell(myHero, _Q) == READY then
	      CastSpell(_Q)
	      DelayAction(function() AttackUnit(unit) end, delay)
	  end
	end
end ]]--
...
function Rengar:AutoQ()
	...
	if Rengo.Combo.combo_mode:Value() == 2 then
		...
		if ValidTarget(target, AArange+50) then
		    if CanUseSpell(myHero, _Q) == READY then
		      DelayAction(function() CastSpell(_Q) end, 40)
		      DelayAction(function() AttackUnit(target) end, 10)
		  end
		end
...
function Rengar:CastE(unit, delay, validrange)
  if ValidTarget(unit, validrange) then
    if CanUseSpell(myHero, _E) == READY then
    	DelayAction(function() CastSkillShot(_E, EPred.PredPos.x, EPred.PredPos.y, EPred.PredPos.z) end, delay)
    end
  end
end
...
function Rengar:CastW(unit, delay, validrange)
  if ValidTarget(unit, validrange) then
    if CanUseSpell(myHero, _W) == READY then
     DelayAction(function() CastSpell(_W) end, delay)
    end
	end
end
...
if ValidTarget(unit, validrange) then
	  if CanUseSpell(myHero, Hydra) == READY and Hydra ~= 0 then
	    DelayAction(function() CastSpell(Hydra) end, delay)
	  elseif CanUseSpell(myHero, Tiamat) == READY and Tiamat ~= 0 then
	    DelayAction(function() CastSpell(Tiamat) end, delay)
	  end
	end
end
...
function Rengar:Combo()
	...
	if CanUseSpell(myHero, _R) == READYNONCAST and GotBuff(myHero,"RengarR") then
		Rpressed = 1
		if CanUseSpell(myHero, YG) == READY and YG ~= 0 then DelayAction(function() CastSpell(YG) end, 10) end
  ...
  if GetCurrentMana(myHero) <= 4 then
    for m, minion in pairs(minionManager.objects) do
      if GetTeam(minion) == MINION_ENEMY then
        if IsReady(_Q) and ValidTarget(minion, 200) then
          CastSpell(_Q)
        	IOW:ResetAA()
        end
        if IsReady(_W) and ValidTarget(minion, 200) then
          CastSpell(_W)
        end
...
function Rengar:JungleClear()
  if GetCurrentMana(myHero) <= 4 then
    for j,mob in pairs(minionManager.objects) do
      if GetTeam(mob) == MINION_JUNGLE then
        if IsReady(_W) and ValidTarget(mob, 200) then
          CastSpell(_W)
        end
        if IsReady(_Q) and ValidTarget(mob, 125) then
          CastSpell(_Q)
          IOW:ResetAA()
        end
        if ValidTarget(mob, GetCastRange(myHero,_E)) then
			MinionPos = GetOrigin(mob)
				if CanUseSpell(myHero, _E) == READY then
					CastSkillShot(_E,MinionPos.x,MinionPos.y,MinionPos.z)
				end
        end
