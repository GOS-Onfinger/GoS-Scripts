...
function LastHitOnTick()
	...
	if ezLastMenu.TurnOffRange:Value() and ezLastMenu.TurnOffRange:Value() > 0 then
		
		local foundEnemy = false
		for i=1, heroManager.iCount do
			local hero = heroManager:getHero(i)
	
			if hero.team == TEAM_ENEMY and ValidTarget(hero) and GetDistance(myHero, hero) < ezLastMenu.TurnOffRange:Value() then
				ezLastMenu.LastHitOn:Value(0) --= false
				foundEnemy = true
				break
			end	
		end
		
		if ezLastMenu.AutoTurnOn:Value() and foundEnemy == false then
			ezLastMenu.LastHitOn:Value(1)-- = true
		end
	end
	
	if me.recalling and me.recallPos and GetDistance(me.recallPos) > 25 then
		me.recalling = false
		ezLastMenu.LastHitOn = true
	end
	...
	if ezLastMenu.ComboKey:Value() and Menu.Orbwalk.AttackChampions:Value() then
		if GetTick() > me.lastWindup
			and not me.recalling
			and GetTick() - lastMoveCommand > movePerSec*1000 then
		
		if GetDistance(mousePos) < Menu.Orbwalk.OrbWalkHoldZone.ezLastMenu:Value() then
...
function OnLoad()	
		...
		ezLastMenu = MenuConfig("ezLastHit v" .. sVersion, "ezLastHit" .. sVersion)
		ezLastMenu:KeyBinding("ComboKey", "Combo Key", comboKey)
		ezLastMenu:KeyBinding("LaneClearOn", "LaneClear Mode", LaneClearKey)
		ezLastMenu:KeyBinding("ForceLastHit", "Force LastHit", LastHitKey)
		ezLastMenu:KeyBinding("AutoTurnOn", "Auto turn on", TurnOffKey)
		ezLastMenu:Boolean("LastHitOn", "LastHit On", false)
		
		ezLastMenu:Boolean("DrawRangeCircle", "Draw Range Circle", true)
		
		ezLastMenu:Boolean("BlockMovement", "Block Movement when LastHitting (VIP)", SCRIPT_PARAM_ONOFF, true)
		ezLastMenu:Slider("TurnOffRange", "LastHit Range", 700, 0, 2000)
				
		ezLastMenu:Menu("LastHitSettings","LastHit Delay")
		ezLastMenu.LastHitSettings:Slider("LastHitDelay", "LastHit Delay", 0, -200, 200)
		ezLastMenu.LastHitSettings:Boolean("LatencyDelay", "Calculate Ping time", true)
		ezLastMenu.LastHitSettings:Boolean("UnderTowers", "Stop LastHit under tower", false)
		ezLastMenu.LastHitSettings:Boolean("StopComboKey", "Stop LastHit on ComboKey", false)
		
		ezLastMenu:Menu("Orbwalk", "Orbwalk Settings")
		ezLastMenu.Orbwalk:Slider("OrbwalkDelay", "Orbwalk Delay", 50, -100, 300)
		ezLastMenu.Orbwalk:Boolean("AttackChampions", "Attack Champions on ComboKey", false)
		--Menu.Orbwalk:addParam("HarassChampions", "Attack Champions on LastHit", SCRIPT_PARAM_ONOFF, false)
		ezLastMenu.Orbwalk:Boolean("OrbWalk", "Orbwalk when Lasthitting", true)
		ezLastMenu.Orbwalk:Boolean("RestartAuto", "Restart Canceled Auto", false)
		ezLastMenu.Orbwalk:Slider("OrbWalkHoldZone", "Orbwalk Hold Zone", 0, 125,700)
		
		
		ezLastMenu.LastHitOn = false
		ezLastMenu.AutoTurnOn = false
		ezLastMenu.LaneClearOn = false
		ezLastMenu.ForceLastHit = false
		
		--ezLastMenu:permaShow("AutoTurnOn")
		--ezLastMenu:permaShow("LastHitOn")
		--ezLastMenu:permaShow("ForceLastHit")
		--ezLastMenu:permaShow("LaneClearOn")		
				
		ezLastMenu:Menu("Mastery", "Mastery")
		ezLastMenu.Mastery:Boolean("DevestatingStrike", "DevestatingStrike", true)
		ezLastMenu.Mastery:Boolean("HavocOn", "HavocOn", true)
		ezLastMenu.Mastery:Boolean("DEdgedSwordOn", "DEdgedSwordOn", true)
		ezLastMenu.Mastery:Boolean("ButcherOn", "ButcherOn", true)
		ezLastMenu.Mastery:Boolean("ArcaneBladeOn", "ArcaneBladeOn", true)
